...
function allSongsAreOverTwoMinutes(songs) {
  let minutes = songs.every((song) => {song.runtimeInSeconds > 120})
  return minutes
}
...
function anySongIsOverFourMinutes(songs) {
  let minutes = songs.some((song) => song.runtimeInSeconds > 240)
  return minutes
}
...
function anySongIsByPeanut(songs) {
 let peanut = songs.some((song) => song.artist === "Peanut");
  return peanut
}
